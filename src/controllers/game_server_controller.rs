            ...
            AsyncCommands::hget::<_, _, String>(&mut redis_conn, &key_list, "leader").await
        {
            ...
            if let Ok(listener) = tokio::net::TcpListener::bind("0.0.0.0:0").await {
                if let Ok(address) = listener.local_addr() {
                    ...
                    if let Ok(exec) =
                        Command::new(r"D:\GameBuilds\WindowsServer\BeatHimUpServer.exe")
                    ...
                    return (StatusCode::SERVICE_UNAVAILABLE, "No port available !")
                        .into_response();
                }
            } else {
...
pub async fn drop_game_server(
    ...
    if let Some(current_lobby_id) = query_payload.get("server_id") {
        let key_list = format!("lobby:{}", current_lobby_id);
        let game_server_info_key = format!("game_server:{}", current_lobby_id);
        if let Ok(_) = AsyncCommands::del::<_, ()>(&mut redis_conn, game_server_info_key).await {
            if let Ok(_) =
                AsyncCommands::hset::<_, _, _, ()>(&mut redis_conn, &key_list, "status", "Ready")
                    .await
            {
